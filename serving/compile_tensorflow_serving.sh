...
if [ -z $TENSORFLOW_SERVING_REPO_PATH ]; then
	TENSORFLOW_SERVING_REPO_PATH="tf-data-processor/tf_serving"
fi
INITIAL_PATH=$(pwd)
...
bazel build -c opt --config=cuda --copt=-mavx --copt=-mavx2 --copt=-mfma --copt=-mfpmath=both --copt=-msse4.2 --cxxopt="-D_GLIBCXX_USE_CXX11_ABI=0" --spawn_strategy=standalone //tensorflow_serving/model_servers:tensorflow_model_server
